...
class Test(TestCase):
    ...
    def test_compute_challenge_error(self):
        ...
        self.assertEqual(dec(1), compute_challenge_error(CHALLENGE_NUMBER, [dec(0), dec(1), dec(0), dec(1)],
                                                         [dec(1), dec(0), dec(1), dec(0)]))

        # intermediate RMSE just for sake of completeness
        self.assertEqual(dec(np.sqrt(0.5)), compute_challenge_error(CHALLENGE_NUMBER, [dec(0), dec(1), dec(0), dec(1)],
                                                                    [dec(0), dec(0), dec(1), dec(1)]))

        # minimum RMSE is 0 when series are identical
        self.assertEqual(dec(0), compute_challenge_error(CHALLENGE_NUMBER, [dec(0), dec(1), dec(0), dec(1)],
    ...
    def test_compute_challenge_scores(self):
        ...
        self.assertEqual(compute_challenge_scores(CHALLENGE_NUMBER, []), [])

        # simple case where errors are all different
        errors = [0, 0.8, 0.2, 1]
        ranks = [1, 3, 2, 4]
        scores = [1 - (r - 1) / (len(ranks) - 1) for r in ranks]
        for x, y in zip(compute_challenge_scores(CHALLENGE_NUMBER, errors), scores):
    ...
    def test_compute_challenge_rewards(self):
        ...
        challenge_rewards = compute_challenge_rewards(CHALLENGE_NUMBER, challenge_scores, challenge_pool)
    ...
    def test_compute_competition_rewards(self):
        ...
        competition_rewards = compute_competition_rewards(CHALLENGE_NUMBER, competition_scores, competition_pool)
    ...
    def test_compute_stake_rewards(self):
        ...
        stake_rewards = compute_stake_rewards(CHALLENGE_NUMBER, stakes, stake_pool)
        self.assertEqual(expected_rewards, stake_rewards)

    def test_compute_challenge_pool(self):
        self.assertEqual(dec(0), compute_challenge_pool(CHALLENGE_NUMBER, 0))
        self.assertEqual(dec(1560), compute_challenge_pool(CHALLENGE_NUMBER, 39))
        self.assertEqual(dec(40000), compute_challenge_pool(CHALLENGE_NUMBER, 1000))

    def test_compute_competition_pool(self):
        self.assertEqual(dec(0), compute_competition_pool(CHALLENGE_NUMBER, 0))
        self.assertEqual(dec(4680), compute_competition_pool(CHALLENGE_NUMBER, 39))
        self.assertEqual(dec(120000), compute_competition_pool(CHALLENGE_NUMBER, 1000))

    def test_compute_stake_pool(self):
        self.assertEqual(dec(0), compute_stake_pool(CHALLENGE_NUMBER, 0))
        self.assertEqual(dec(1960), compute_stake_pool(CHALLENGE_NUMBER, 49))
        self.assertEqual(dec(40000), compute_stake_pool(CHALLENGE_NUMBER, 1000))

    def test_compute_pool_surplus(self):
        self.assertEqual(dec(200000), compute_pool_surplus(CHALLENGE_NUMBER, 0, 0))
        self.assertEqual(dec(191800), compute_pool_surplus(CHALLENGE_NUMBER, 39, 49))
        self.assertEqual(dec(0), compute_pool_surplus(CHALLENGE_NUMBER, 1000, 1000))
